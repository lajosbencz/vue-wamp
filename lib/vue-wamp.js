...
function _update(clear) {
  if(_connection) {
    _isConnected = _connection.isConnected;
    _isOpen = _connection.isOpen;
    _isRetrying = _connection.isRetrying;
    console.log('updating...',{_isConnected,_isOpen,_isRetrying});
  }
  if(clear && _interval) {
...
function plugin(Vue, options) {
  ...
  Vue.mixin({
    computed: {
      '$wampIsConnected'() { return _isConnected },
      '$wampIsOpen'() { return _isOpen },
      '$wampIsRetrying'() { return _isRetrying },
    },
    created() {
